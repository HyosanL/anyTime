...
> - reviews.js had a raw NUL byte (onReviewWritten map key) → escaped to a unicode
>   escape during Task 2 so the file reads as text.
> - Firestore rules validated by CI only (Firestore emulator needs Java, absent here).
> - Terraform not `plan`-validated locally (terraform absent) — HCL bracket-checked; `plan` is the user's gate per infra/cloudflare/README.md.
> - `_middleware.js`: `/api/board-sweep` now also accepts `X-Push-Secret` (for `boardImageSweep`).
