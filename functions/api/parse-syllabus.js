...
// Flash-Lite(6배 저렴) 로 내렸다가 Flash 로 되돌린다. 표 텍스트 → 행 추출이 프런티어 추론을
// 요구하진 않지만, 한 줄에 분반·교수·시간이 여러 벌 늘어선 표(영어회화 4·5·6분반이 같은
// 목1교시에 교수만 다르게 열리는 식)에서 Flash-Lite 가 열 정렬을 놓치고 교수 한 명을 세
// 분반에 모두 붙였다. 잘못 적재된 시간표를 손으로 고치는 비용이 파싱 비용보다 크다.
// 1회 분석(~19콜) 약 $0.43. 재분석은 캐시라 무과금이고, 실제 호출은 학기당 몇 번뿐이다.
// 더 싸게(또는 더 세게) 가고 싶으면 코드 수정 없이 Pages 환경변수 GEMINI_MODEL 로 갈아끼운다.
