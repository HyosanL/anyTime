...
- "제 2 외국어 1( 러 )" 처럼 **"제2외국어" 바로 뒤의 숫자는 분반 번호가 아니라 과목명의
  일부(단계)**다 → course "제2외국어1(러)"(그 뒤에 나오는 진짜 분반 번호를 sectionNo 로).
  "제 2 외국어 2( 독 )" 도 마찬가지로 course "제2외국어2(독)". 언어 표기는 표에 적힌 축약형
  그대로("러","독","일") 두고 "러시아어"처럼 풀어쓰지 마라 — 기존 DB가 축약형으로 등록돼
  있어 풀어쓰면 같은 과목이 새 과목으로 중복 등록된다.
...
- 이 세부표 페이지에는 "필","택","수","전","공","컴","퓨","터" 처럼 **한 글자씩 세로로
  ...
  것) — 이런 한두 글자짜리 고립된 줄은 무시해라. 과목명 뒤로 분반·시간·교수 숫자가 하나도
  안 붙어 있으면(같은 블록에 번호로 시작하는 줄이 없으면) 그 과목은 이 페이지에 실제
  개설 정보가 없는 것이니 행을 만들지 마라(분반을 지어내지 말 것).
