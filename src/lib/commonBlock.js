...
//
//  DB(custom_class)에 담지 않는다 — 모두에게 똑같은 시간이라 계정마다 저장할 이유가 없다.
//  담으면 ① 생도 수만큼 쓰기가 생기고 ② 학기가 바뀔 때마다 옛 항목이 남으며
//  ③ 마법사의 '빈 시간표 재사용' 판정이 깨진다(강의는 없는데 항목이 있으니 안 비어 보인다).
//  대신 카탈로그(common_block)에서 그때그때 계산해 격자에만 깐다.
//
...
// common_block(교시 단위) → 격자가 쓰는 분(minute) 구간. 교시 시각(period)이 없으면 못 그린다.
