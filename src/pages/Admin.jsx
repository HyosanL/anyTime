...
export default function Admin() {
  ...
  return (
    <div className="page admin">
      ...
      {msg && <p className={`admin-msg ${isFail ? 'is-fail' : 'is-ok'}`}>{msg}</p>}
      ...
      <div className="cards admin-cards">
                ...
                {selCourse === c.code && (
                  <div className="adm-expand">
                    ...
                    <div className="adm-form-grid">
                      ...
                      <label className="field"><span className="field-label">시작교시</span><input type="number" value={st.start_period} onChange={(e) => setSt({ ...st, start_period: +e.target.value })} /></label>
                      <label className="field"><span className="field-label">끝교시</span><input type="number" value={st.end_period} onChange={(e) => setSt({ ...st, end_period: +e.target.value })} /></label>
                      <label className="field"><span className="field-label">강의실</span><input placeholder="예: 본관 201" value={st.room} onChange={(e) => setSt({ ...st, room: e.target.value })} /></label>
                    </div>
                    <button className="btn-add btn-block" onClick={() => run('set_section_time', { course_code: c.code, year: sec.year, term: sec.term, section_no: sec.section_no, ...st }, '강의시간 추가')}>강의시간 추가</button>
                  </div>
                )}
              </li>
            ))}
        ...
        {/* 8. 아이디 / 기기 차단 */}
        <Card icon="🚫" title="아이디 / 기기 차단" desc="문제 사용자를 일정 기간 차단합니다. 계정과 기기가 함께 차단되며, 칩에서 해제할 수 있습니다.">
          <div className="section-label adm-sub-label">현재 차단 목록</div>
          <div className="adm-tags">
            {blocks.length === 0 ? <span className="note">차단된 사용자가 없습니다.</span> : blocks.map((b) => (
              <span key={b.id} className="tag tag-warn">{b.username} ~{new Date(b.blocked_until).toLocaleDateString('ko-KR')}
                <button className="x" onClick={() => run('unblock', { username: b.username }, '차단 해제')}>×</button>
              </span>
            ))}
          </div>

          <div className="section-label adm-sub-label">새 차단</div>
          <div className="adm-form-grid">
            <label className="field"><span className="field-label">아이디</span><input placeholder="차단할 아이디" value={blk.username} onChange={(e) => setBlk({ ...blk, username: e.target.value })} /></label>
            <label className="field"><span className="field-label">일수</span><input type="number" value={blk.days} onChange={(e) => setBlk({ ...blk, days: +e.target.value })} /></label>
            <label className="field adm-field-wide"><span className="field-label">사유 (선택)</span><input placeholder="사유" value={blk.reason} onChange={(e) => setBlk({ ...blk, reason: e.target.value })} /></label>
          </div>
          <button className="btn-danger-soft btn-block" onClick={async () => { if (!blk.username.trim()) return; const r = await run('block_user', blk, `${blk.username} ${blk.days}일 차단`); if (r.ok) setBlk({ username: '', days: 7, reason: '' }); }}>차단 (계정 + 기기)</button>
        </Card>
